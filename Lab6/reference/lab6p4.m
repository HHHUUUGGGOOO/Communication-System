...
for i = 0:2:20
    ...
    bin_seq = MD_symbol_demapper(sym_seq, M, d, 'PSK');
    decoded_data = conv_dec(bin_seq, impulse_response);
    ...
    for i = 2:length(V)
        for j = 1:statenum
            if s(i-1,j) ~= 0
                ...
                p1 = s1*2-mod(j,2);
                p2 = s2*2-mod(j,2);
                if (V(i,s1) == 0 && change(i,s1) == 0)|| V(i,s1) > V(i-1,j)+sum(binary_data((i-2)*K+1:(i-1)*K) ~= all_coded(p1,:))
                    V(i,s1) = V(i-1,j)+sum(binary_data((i-2)*K+1:(i-1)*K) ~= all_coded(p1,:));
                    s(i,s1) = j;
                    change(i,s1) = change(i,s1)+1;
                end
                if (V(i,s2) == 0 && change(i,s2) == 0) || V(i,s2) > V(i-1,j)+sum(binary_data((i-2)*K+1:(i-1)*K) ~= all_coded(p2,:))
                    V(i,s2) = V(i-1,j)+sum(binary_data((i-2)*K+1:(i-1)*K) ~= all_coded(p2,:));
                    s(i,s2) = j;
